...
/**
 * Starts authenticated session with RFID tag
 *
 * This must be called before any read operation on the tag
 */
bool RFID_start_auth(RFID *RFID);

/**
 * Stops authenticated session with RFID tag
 *
 * This must be called to end an authenticated session, otherwise no RFID tags
 * can be detected
 */
