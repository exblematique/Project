...
from module import DefaultModule, ConnectionModule, TransformerModule, WireModule, ImportExportModule
...
class FlowSegment(object):
    ...
    def get_byte(self):
        ...
        speed = self.speed << 5
        direction = (
            self.direction if self.direction != None else Direction.FORWARDS) << 4
        load = self.load << 2
        state = self.state
    ...
    def __repr__(self):
        return 'FlowSegment({0}, {1}) [s {2},v {3},l {4}]'.format(self.start_pos, self.end_pos, self.state, self.voltage, self.load)
