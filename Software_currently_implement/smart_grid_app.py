...
class SmartGridApp(object):
    ...
    def __init__(self, serial_port, baudrate):
        ...
        self.smart_grid_table = SmartGridTable(self.grid, self.add_message)
        ...
        self.websocket_events = {
            MessageTypes.CALCULATION_FINISHED,
            MessageTypes.CONFIG_CHANGED,
            MessageTypes.COLOR_CHANGED,
            MessageTypes.POWER_BOUNDARIES_CHANGED,
            MessageTypes.FLOW_DISABLED,
            MessageTypes.TABLE_CONNECTED,
            MessageTypes.RESET_TABLES
        }
        ...
        self.gateway_conn = GatewayConnector(
            self.add_message, serial_port, baudrate)
        start_daemon_thread(self.gateway_conn.start_serial_read, ())
        log('--------GW CONN STARTED')

        # Start websocket
        self.websocket = Websocket(self.websocket_events)
        self.websocket.start('0.0.0.0', 9001)
    ...
    def handle_messages(self):
        ...
        while 1:
            ...
            # If message type is in the websocket events list, send socket message
            if message.type in list(self.websocket_events):
                self.websocket.send_data_message(message.type, message.args)

            # Execute message functions
            if message.type in list(self.message_functions.keys()):
    ...
    @staticmethod
    def power_boundaries_changed(*args):
        """
